...
Using ~emacssql~ I created the following table.
...
I use a simplified SM2 algorithm to calculate the intervals.

\(\text{New Interval} = \text{round}(\text{Old Interval} \times
\text{A Factor})\)
